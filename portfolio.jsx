...
const PROJECTS = [
  {
    ...
    detail: {
      ...
      outcomes: `COLOBLOC is a live generative installation built on a real-time system framework known as the Chaos Engine. The work visualizes how external pressure and collective engagement interact over time by mapping these forces onto a dynamic field of agents representing fragments of place. Each agents state—stability, decay, disappearance, or reseeding—is continuously recalculated based on shifting inputs, including simulated conditions and live interaction.

      The system operates as a continuous loop rather than a fixed sequence, allowing the environment to evolve responsively. Viewer interaction redistributes stability across the field, producing asymmetrical outcomes that reflect the uneven relationship between attention and structural force. The installation supports multi-user engagement, enabling collective behavior to influence system dynamics in real time.

      COLOBLOC extends the Chaos Engine into a spatial, experiential context, integrating visual output, system messaging, and potential data inputs into a cohesive environment. Designed for adaptability, the work can incorporate publicly available datasets—such as housing, development, and economic data—to ground system behavior in local conditions.`,
  ...
  },
  {
    ...
    detail: {
      longDesc: "A high-contrast, text-driven motion piece that uses rule-based sequencing to cycle through conflicting statements of authorship, creating a continuous loop of assertion and collapse. The work operates as a system rather than a narrative, with subtle visual instability reinforcing the breakdown of a singular creator.",
      process: [
        "DEFINE THE CONCEPTUAL SYSTEM - Begin by clearly establishing the internal logic of the piece before any visual work is created. identify the core phrases and determine how they relate, contradict, and destabilize one another. This step is about building a rule-based system where authorship is continuously asserted and denied, creating a loop of unresolved meaning rather than a fixed message.",
        "DEVELOP THE VISUAL LANGUAGE - Design a visual approach that reinforces clarity, tension, and scale. Focus on high contrast compositions, bold typography, and minimal elements so each phrase can be read instantly. Establish a consistent aesthetic dark fields, controlled highlights, subtle glow or distortion that supports the feeling of a system rather than a decorative design. The goal is to create a visual language that is both legible and atmospheric.",
        "BUILD THE MOTION BEHAVIOR - Animate the work as a living process rather than a sequence of static scenes. Introduce slow, deliberate timing with subtle variations such as flicker, drift, or fragmentation. Text should appear, shift, and reorganize in ways that suggest an underlying system attempting to correct or process information. The motion should feel continuous and slightly unstable, as if the piece is always in the middle of becoming.",
        "STRUCTURE THE LOOPING SEQUENCE - Organize the piece into a seamless loop that moves through phases of declaration, contradiction, and system expansion. Begin with a clear statement, allow it to break down into opposing phrases, and then transition into broader system-level language before condensing into the final combined line. Ensure that the ending naturally feeds back into the beginning, creating a continuous cycle with no clear start or finish.",
        "RENDER THE FINAL WORK - Export the completed piece as a clean, high-quality video with consistent pacing and no external elements. Ensure that all visuals, text, and motion are original and cohesive, and that the loop plays smoothly without interruption. This final step focuses on refining the work into a polished, self-contained system that can stand on its own without explanation.",
      ],
      outcomes: "The work is a text-driven motion system that cycles through a predefined set of phrases using a rule-based sequencing and timed transitions. Typography is rendered in high contrast with controlled visual effects such as glow, displacement, flicker, and opacity shifts to introduce subtle instability. Animation is governed by a loop structrure (~170 seconds) composed of phased states where elements appear, reorganize, and dissolve according to timing rules rather than user interaction. The sytstem may be built using procedural tools or timeline based software but is ultimely rendered as a seamless, silent video file. All asset are original, and the final output maintains consistent resolution, pacing, and legibiity across the full duration of the loop.",
